...
class MotorControlGUI:
    ...
    def save_motion(self):
        ...
        motion_block = {
            ...
            "motor_speeds": {str(i + 3): motor_speeds[i] for i in range(len(motor_speeds))},       # 모터 ID 3~12
            "motor_positions": {str(i + 3): motor_positions[i] for i in range(len(motor_positions))},
            "servo_angles": {str(k): v for k, v in zip([1,2,3,4,6,7,8,9], servo_angles)}
        }
        motions = []
        if os.path.exists(filename):
            try:
                ...
                print(f"⚠️ 기존 파일 로딩 실패 (새 파일로 간주): {e}")

        # 6. motion 추가 및 저장
        motions.append(motion_block)

        try:
            with open(filename, "w") as f:
                json.dump(motions, f, indent=2, separators=(", ", ": "))
            print(f"✅ Motion data appended to {filename}")
        except Exception as e:
