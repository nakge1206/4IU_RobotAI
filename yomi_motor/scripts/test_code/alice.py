...
class MotorControlGUI:
    def __init__(self, root):
        ...
        # 퍼블리셔 생성 (모터 속도 변경)
        self.motor_position_pub = rospy.Publisher('/motor_position_cmd', Int16MultiArray, queue_size=12)


        # 현재 모터 속도를 가져오는 요청 퍼블리셔
        self.motor_position_request_pub = rospy.Publisher('/motor_position_request', Int16MultiArray, queue_size=12)

        # 모터 속도 데이터 저장
        self.motor_position = [0] * 12 
        self.initial_length = 12  # 기본적으로 10개의 모터가 있다고 가정

        # GUI 요소 생성
        tk.Label(root, text="Motor Speed Control", font=("Arial", 14)).grid(row=0, columnspan=4)

        # "Load Motor Speeds" 버튼 (현재 값 불러오기)
        self.load_button = tk.Button(root, text="Load Motor Speeds", command=self.request_motor_position)
        ...
        # "적용" 버튼 (수정된 값 반영)
        self.apply_button = tk.Button(root, text="Apply Changes", command=self.apply_motor_position)

        self.apply_button.grid(row=self.initial_length + 2, column=0, columnspan=4, pady=10)

        # ROS 구독 설정 (모터 속도 업데이트)
        rospy.Subscriber('/motor_position_fb', Int16MultiArray, self.update_motor_position)
    ...
    def create_sliders(self):
        ...
        for i in range(self.initial_length):
            ...
            # 슬라이더
            slider = tk.Scale(self.root, from_=0, to=5, orient="horizontal")
    ...
    def update_motor_position(self, msg):
        ...
        # 불러온 값을 슬라이더에 적용
        for i in range(self.initial_length):
            self.motor_position[i] = new_data[i]  # 리스트 업데이트
            self.sliders[i].set(new_data[i])  # 슬라이더 UI 업데이트

        print("Motor Speeds Updated:", self.motor_position)

    def request_motor_position(self):
        """아두이노에 현재 모터 속도 요청"""
        msg = Int16MultiArray()
        msg.data = []  # 데이터 없이 요청만 보냄
        self.motor_position_request_pub.publish(msg)
        print("Requested current motor position from Arduino.")

    def apply_motor_position(self):
        """슬라이더 값을 읽어 모터 속도를 업데이트하고 ROS 토픽 발행"""
        for i in range(self.initial_length):
            self.motor_position[i] = self.sliders[i].get()  # 슬라이더에서 값 가져오기

        msg = Int16MultiArray()
        msg.data = self.motor_position[:]  # 리스트 데이터 복사하여 발행
        self.motor_position_pub.publish(msg)

        print("Applied Motor Speeds:", self.motor_position)
